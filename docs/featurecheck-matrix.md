...
| --- | --- | --- | --- | --- |
| A | Template creation | Ready | `generator::create_project`, built-in project templates, `RadsProject::starter`, and `templates::*` write the starter tree. See `docs/template-creation.md`. | Generate a temp project and require `rads.project.json`, `app.blueprint.json`, `ui/main.ui2`, `ui/index.html`, `ui/styles.css`, Rust stubs, package manifests, and README. |
...
| F | BP DIST stages | Partial | `JobKind::Pack` simulates blueprint validation, UI2 layout collection, and package-plan writing. `check`, `build`, `pack`, and `auto` map to BP DIST stages. See `docs/bp-dist-stages.md`. | Spawn a public pack job and require the BP DIST stage log lines. |
...
- Replace simulated package output with a BP DIST artifact once the TRUEOS
  package writer is available.
